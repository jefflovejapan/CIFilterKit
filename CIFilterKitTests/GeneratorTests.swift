    ...
    override func spec() {
        describe("The AztecCodeGenerator filter") {
            it("should be able to generate an image") {
                let data = "o hai Leslie".data(using: .utf8)
                var outImg: CIImage?
                if let data = data {
                    outImg = AztecCodeGenerator(inputMessage: data, options:AztecCodeGeneratorOptions())
                }
        ...
        describe("The CheckerboardGenerator filter") {
            it("should be able to generate an image") {
                ...
                let outImg = CheckerboardGenerator(options: options)
                expect(outImg).toNot(beNil())
            }
        }
        describe("The Code128BarcodeGenerator filter") {
            it("should be able to generate an image") {
                let outImg = Code128BarcodeGenerator(inputMessage: "blahblahsomemessage".data(using: .ascii)!, inputQuietSpace:nil)
                expect(outImg).toNot(beNil())
            }
        }
        ...
        describe("The QRCodeGenerator filter") {
            it("should be able to generate an image") {
                let outImg = QRCodeGenerator(inputMessage: "Good morning".data(using: .utf8)!, inputCorrectionLevel:.M)
                expect(outImg).toNot(beNil())
            }
        }
        ...
        describe("The StarShineGenerator filter") {
            it("should be able to generate an image") {
                ...
                let outImg = StarShineGenerator(options: options)
                expect(outImg).toNot(beNil())
            }
        }
        describe("The StripesGenerator filter") {
            it("should be able to generate an image") {
                ...
                let outImg = StripesGenerator(options: options)
                expect(outImg).toNot(beNil())
            }
        }
    }
